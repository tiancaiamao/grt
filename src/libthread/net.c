...
netannounce(int istcp, char *server, int port)
{
	...
	fdnoblock(fd);
	threadsetstate("netannounce succeeded");
	return fd;
}
...
netaccept(int fd, char *server, int *port)
{
	...
	threadsetstate("netaccept");
	len = sizeof sa;
	if((cfd = accept(fd, (void*)&sa, &len)) < 0){
		threadsetstate("accept failed");
		return -1;
	}
	if(server){
	...
	threadsetstate("netaccept succeeded");
	return cfd;
}
...
netlookup(char *name, uint32_t *ip)
{
	...
	threadsetstate("netlookup");
	if((he = gethostbyname(name)) != 0){
		*ip = *(uint32_t*)he->h_addr;
		threadsetstate("netlookup succeeded");
		return 0;
	}
...
netdial(int istcp, char *server, int port)
{
	...
	if(getpeername(fd, (struct sockaddr*)&sa, &sn) >= 0){
		threadsetstate("connect succeeded");
		return fd;
	}
